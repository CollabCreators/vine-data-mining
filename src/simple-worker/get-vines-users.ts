...
export default class GetVinesUsers {
  ...
  public beginUsersWithExisting(): void {
    this.readExistingUsers()
      .then((users) => {
      this.users = users;
      this.jobEventEmitter.on("job.existing.done", () => this.doNextUserJob());
      this.doNextUserJob();
    });
  }
  ...
  private doNextUserJob(): void {
    ...
    console.log("Begin", timeLabel, ++this.counter);
    console.time(timeLabel);
    this.vineApi.getUserProfile(userData.id).then((data) => {
      if (!data) {
        return Promise.resolve();
      }
      console.log(`stored user ${userData.id}`);
      console.timeEnd(timeLabel);
      return this.localStorage.storeParsedData(data);
    })
    // Catch any errors in chain.
      .catch((err) => console.error(err.stack))
    // Emit done event (always, even if there was an error).
